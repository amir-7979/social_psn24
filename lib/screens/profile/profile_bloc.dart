...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  FutureOr<void> _onFetchProfileEvent(FetchProfileEvent event, Emitter<ProfileState> emit) async {
  ...
  void _onChangeToPostEvent(ChangeToPostEvent event, Emitter<ProfileState> emit) => emit(ChangeToPostState());

  void _onChangeToCommentEvent(ChangeToCommentEvent event, Emitter<ProfileState> emit) => emit(ChangeToCommentState());

  FutureOr<void> _onEditPostEvent(EditPostEvent event, Emitter<ProfileState> emit) {}
  ...
  FutureOr<void> handleChangeStatusEvent(ChangeStatusEvent event, Emitter<ProfileState> emit) async {
    try {
      final gql.MutationOptions options = changeOnlineStatus();
      final gql.QueryResult result = await graphQLService.mutate(options);
      if (result.hasException) {
